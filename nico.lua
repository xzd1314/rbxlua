...
-- ===== 内联库代码（由 Python 生成器自动填充）=====
local Fluent, FluentErr = loadstring([==[
--[[
...
if not Fluent then
    error("Fluent 库加载失败: " .. tostring(FluentErr))
end
Fluent = Fluent()

local SaveManager, SMErr = loadstring([==[
...
if not SaveManager then
    error("SaveManager 库加载失败: " .. tostring(SMErr))
end
SaveManager = SaveManager()

local InterfaceManager, IMErr = loadstring([==[
local httpService = game:GetService("HttpService")
...
if not InterfaceManager then
    error("InterfaceManager 库加载失败: " .. tostring(IMErr))
end
...
if not bots_folder then
    ...
    local function createPlatformAtPlayer()
        local char = LocalPlayer.Character
        if not char then return end
        local root = char:FindFirstChild("HumanoidRootPart")
        if not root then return end
        local pos = root.Position
        local platformY = pos.Y - 50
        if platformPart then platformPart:Destroy() end
        platformPart = Instance.new("Part")
        platformPart.Size = Vector3.new(800, 1, 800)
        platformPart.Position = Vector3.new(pos.X, platformY, pos.Z)
        platformPart.Anchored = true
        platformPart.CanCollide = true
        platformPart.Transparency = 0
        platformPart.Material = Enum.Material.Concrete
        platformPart.BrickColor = BrickColor.new("Medium stone gray")
        platformPart.Name = "UserPlatform"
    ...
    local function teleportToPlatform()
        ...
        local pos = platformPart.Position
    ...
    local platformToggle = Tabs.Main:AddToggle("PlatformToggle", {
        Title = "生成平台",
        Default = false,
    ...
    Tabs.Main:AddButton({Title = "传送至平台", Callback = teleportToPlatform})
    Tabs.Main:AddButton({Title = "传送回原位", Callback = teleportToSaved})

    -- ========== 提示信息 ==========
    Tabs.Main:AddParagraph({
        Title = "使用方法",
        Content = "先开启 Bot 透视观察 Bot 位置。按 F8 启动自动嘲讽跳（强制速度100、连点30、重力200，关闭自动恢复原值）；按 F7 启动边跳舞边移动（强制速度100、重力默认，长按W+C+空格）。两者互斥。"
    })
    ...
    RunService:BindToRenderStep("GermanizedLoop", Enum.RenderPriority.Camera.Value + 1, function(dt)
        pcall(function()
            ...
            local walkSpeed = Options.SpeedChanger.Value
